...
bool move_robot(double x, double y, double z, double yaw, float angle_error) {
	...
	static double image_interval = 1.0;
...
void odomCallback(
    ...
    const ImuConstPtr& imu,
	const OdometryConstPtr& wheel,
	const localize::theta_stampedConstPtr &line_angle
	)
{
	//ImageConstPtr image;
	...
	static bool is_take_image = false;	///////////////////////// false
...
//	is_take_image = is_save_pic_data; -- modified
	...
	if(is_take_image){
		...
		cout<<"move_pos : TRUE, picture taken";
	}
}
...
int main(int argc, char** argv)
{
	...
	message_filters::Subscriber<Imu> imu_sub(nh, "/imu", 40);
	message_filters::Subscriber<Odometry> wheel_sub(nh, "/odom", 400);  // wheel encoder
	message_filters::Subscriber<localize::theta_stamped> angle_sub(nh, "/line_angle", 400);
	...
	typedef sync_policies::ApproximateTime<PoseWithCovarianceStamped, Image, Imu, Odometry, localize::theta_stamped> MySyncPolicy;
	Synchronizer<MySyncPolicy> sync(MySyncPolicy(10), odom_sub, image_sub, imu_sub, wheel_sub, angle_sub);
	sync.registerCallback(boost::bind(&odomCallback, _1, _2, _3, _4, _5));
